...
void CTaskbarNotifier::Show(LPCTSTR szCaption, DWORD dwTimeToShow, DWORD dwTimeToLive, DWORD dwTimeToHide, int nIncrement)
{
	...
	switch (m_nAnimStatus)
	{
		...
		case IDT_DISAPPEARING:
			...
			SetWindowPos(CWnd::FromHandle(HWND_TOPMOST), m_nCurrentPosX, m_nCurrentPosY, m_nSkinWidth, m_nSkinHeight, SWP_NOACTIVATE);
			RedrawWindow();
			break;
	}
}
...
void CTaskbarNotifier::OnWindowPosChanging(WINDOWPOS* lpwndpos)
{
